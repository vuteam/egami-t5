...
from boxbranding import getBrandOEM, getBoxType, getMachineBrand
...
def InitUsageConfig():
    ...
    config.workaround.blueswitch = ConfigSelection(default='0', choices=[('0', _('EGAMI Panel/Extensions')), ('1', _('Extensions/EGAMI Panel'))])
    ...
    if fileExists('/usr/lib/enigma2/python/Plugins/Extensions/CoolTVGuide/plugin.pyo'):
        config.usage.okbutton_mode = ConfigSelection(default='0', choices=[('0', _('InfoBar')),
         ('1', _('Channel List')),
        ...
        config.usage.okbutton_mode = ConfigSelection(default='0', choices=[('0', _('InfoBar')), ('1', _('Channel List'))])
    ...
    config.usage.on_movie_stop = ConfigSelection(default='quit', choices=[('ask', _('Ask user')), ('movielist', _('Return to movie list')), ('quit', _('Return to previous service'))])
    ...
    config.usage.leave_movieplayer_onExit = ConfigSelection(default='no', choices=[('no', _('No')),
     ('popup', _('With popup')),
    ...
    config.usage.serviceitems_per_page = ConfigSelectionNumber(default=17, stepwidth=1, min=3, max=40, wraparound=True)
    ...
    config.usage.swap_time_display_on_vfd = ConfigSelection(default='0', choices=[('0', _('Skin Setting')),
     ...
     ('4', _('Hours Mins Secs')),
     ('5', _('Percentage'))])
    config.usage.swap_media_time_display_on_vfd = ConfigSelection(default='2', choices=[('0', _('Skin Setting')),
     ('1', _('Mins')),
     ('2', _('Mins Secs')),
     ('3', _('Hours Mins')),
     ('4', _('Hours Mins Secs')),
    ...
    config.usage.allowed_vod_paths = ConfigLocations(default=[resolveFilename(SCOPE_VOD)])
    config.usage.enableVodMode = ConfigYesNo(default=True)
    config.usage.double_line_mode = ConfigSelection(default='0', choices=[('0', _('Single Line')), ('1', _('Double Line'))])
    config.usage.double_line_mode.addNotifier(refreshServiceList)
...
#    config.usage.slider_graphic = ConfigYesNo(default=False)
    config.usage.enable_hbbtv_autostart = ConfigYesNo(default=True)
    config.usage.async_plug_load = ConfigYesNo(default=False)
    config.usage.skin_selector = ConfigSelection(default='no', choices=[('no', _('No')), ('yes', _('Yes, install new skins'))])
    ...
    config.usage.show_notification_for_updates = ConfigYesNo(default=False)
    config.usage.update_available = NoSave(ConfigYesNo(default=False))
    config.usage.smart_scrollbar_timeout = ConfigInteger(default=3000)
#    config.usage.animation_type = ConfigSelection(default='sbtt', choices=[('srtl', _('Slide right to left')),
...
#    config.usage.animation_type.addNotifier(chaneAnimationType)
    ...
    config.misc.query_restart.value = False
    ...
    if SystemInfo['ZapMode']:
        try:
            if os.path.exists('/proc/stb/video/zapping_mode'):
                ...
                zapoptions = [('mute', _('Black screen')),
                 ('hold', _('Hold screen')),
    ...
    config.subtitles.showbackground = ConfigYesNo(default=True)
    ...
    config.vixsettings.InfoBarEpg_mode = ConfigSelection(default='0', choices=[('0', _('as plugin in extended bar')),
    ...
    config.epgselection.infobar_prevtimeperiod = ConfigSelection(default='180', choices=[('60', _('%d minutes') % 60),
     ('90', _('%d minutes') % 90),
     ('120', _('%d minutes') % 120),
     ('150', _('%d minutes') % 150),
     ('180', _('%d minutes') % 180),
     ...
     ('270', _('%d minutes') % 270),
     ('300', _('%d minutes') % 300)])
    config.epgselection.infobar_primetimehour = ConfigSelectionNumber(default=20, stepwidth=1, min=0, max=23, wraparound=True)
    config.epgselection.infobar_primetimemins = ConfigSelectionNumber(default=0, stepwidth=1, min=0, max=59, wraparound=True)
    ...
    config.epgselection.graph_prevtimeperiod = ConfigSelection(default='180', choices=[('60', _('%d minutes') % 60),
     ('90', _('%d minutes') % 90),
     ('120', _('%d minutes') % 120),
     ('150', _('%d minutes') % 150),
     ('180', _('%d minutes') % 180),
    ...
    config.epgselection.graph_primetimemins = ConfigSelectionNumber(default=0, stepwidth=1, min=0, max=59, wraparound=True)
    config.epgselection.graph_servicetitle_mode = ConfigSelection(default='servicename', choices=[('servicename', _('Service Name')),
     ('picon', _('Picon')),
    ...
    possibleAlignmentChoices = [(str(RT_HALIGN_LEFT | RT_VALIGN_CENTER), _('left')),
    ...
    config.epgselection.graph_rec_icon_height = ConfigSelection(choices=[('bottom', _('bottom')),
     ('top', _('top')),
     ('middle', _('middle')),
     ('hide', _('hide'))], default='bottom')
