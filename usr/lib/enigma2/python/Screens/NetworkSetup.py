...
class NetworkAdapterSelection(Screen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (self.close, _('Exit network interface list')),
         'ok': (self.okbuttonClick, _('Select interface'))})
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (self.close, _('Exit network interface list')),
        ...
        self['DefaultInterfaceAction'] = HelpableActionMap(self, 'ColorActions', {'yellow': (self.setDefaultInterface, [_('Set interface as the default Interface'), _('* Only available if more than one interface is active.')])})
...
class NameserverSetup(Screen, ConfigListScreen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (self.cancel, _('Exit nameserver configuration')),
...
class AdapterSetup(Screen, ConfigListScreen, HelpableScreen):
    ...
    def __init__(self, session, networkinfo, essid = None):
        ...
        self.createConfig()
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (self.keyCancel, _('Exit network adapter configuration')),
         'ok': (self.keySave, _('Activate network adapter configuration'))})
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (self.keyCancel, _('Exit network adapter configuration')),
         'green': (self.keySave, _('Activate network adapter configuration')),
         'blue': (self.KeyBlue, _('Open nameserver configuration'))})
        self['actions'] = NumberActionMap(['SetupActions'], {'ok': self.keySave}, -2)
        self.list = []
    ...
    def createConfig(self):
        ...
        self.ipConfigEntry = NoSave(ConfigIP(default=iNetwork.getAdapterAttribute(self.iface, 'ip')) or [0,
         0,
         0,
         0])
        self.netmaskConfigEntry = NoSave(ConfigIP(default=iNetwork.getAdapterAttribute(self.iface, 'netmask') or [255,
         0,
         0,
         0]))
        if iNetwork.getAdapterAttribute(self.iface, 'gateway'):
        ...
        self.gatewayConfigEntry = NoSave(ConfigIP(default=iNetwork.getAdapterAttribute(self.iface, 'gateway') or [0,
         0,
         0,
         0]))
    ...
    def createSetup(self):
        ...
        if self.activateInterfaceEntry.value:
            ...
            havewol = False
            if SystemInfo['WakeOnLAN'] and getBoxType() not in ('et10000', 'gb800seplus', 'gb800ueplus', 'gbultrase', 'gbultraue', 'gbipbox', 'gbquad', 'gbx1', 'gbx3'):
                havewol = True
            ...
            for p in plugins.getPlugins(PluginDescriptor.WHERE_NETWORKSETUP):
                ...
                if callFnc is not None:
                    if p.__call__.has_key('WlanPluginEntry'):
...
class AdapterSetupConfiguration(Screen, HelpableScreen):
    ...
    def __init__(self, session, iface):
        ...
        self['WizardActions'] = HelpableActionMap(self, 'WizardActions', {'up': (self.up, _('Move up to previous entry')),
         'down': (self.down, _('Move down to next entry')),
         'left': (self.left, _('Move up to first entry')),
         'right': (self.right, _('Move down to last entry'))})
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (self.close, _('Exit network adapter setup menu')),
         'ok': (self.ok, _('Select menu entry'))})
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (self.close, _('Exit network adapter setup menu'))})
        self['actions'] = NumberActionMap(['WizardActions', 'ShortcutActions'], {'ok': self.ok,
         'back': self.close,
         'up': self.up,
         'down': self.down,
         'red': self.close,
    ...
    def queryWirelessDevice(self, iface):
        ...
        except IOError as (error_no, error_str):
...
class NetworkAdapterTest(Screen):
    ...
    def __init__(self, session, iface):
        ...
        self['updown_actions'] = NumberActionMap(['WizardActions', 'ShortcutActions'], {'ok': self.KeyOK,
         'blue': self.KeyOK,
         'up': lambda : self.updownhandler('up'),
         'down': lambda : self.updownhandler('down')}, -2)
        self['shortcuts'] = ActionMap(['ShortcutActions', 'WizardActions'], {'red': self.cancel,
         'back': self.cancel}, -2)
        self['infoshortcuts'] = ActionMap(['ShortcutActions', 'WizardActions'], {'red': self.closeInfo,
         'back': self.closeInfo}, -2)
        self['shortcutsgreen'] = ActionMap(['ShortcutActions'], {'green': self.KeyGreen}, -2)
        self['shortcutsgreen_restart'] = ActionMap(['ShortcutActions'], {'green': self.KeyGreenRestart}, -2)
        self['shortcutsyellow'] = ActionMap(['ShortcutActions'], {'yellow': self.KeyYellow}, -2)
...
class NetworkMountsMenu(Screen, HelpableScreen):
    ...
    def __init__(self, session):
        ...
        self['WizardActions'] = HelpableActionMap(self, 'WizardActions', {'up': (self.up, _('Move up to previous entry')),
         ...
         'left': (self.left, _('Move up to first entry')),
         'right': (self.right, _('Move down to last entry'))})
        self['OkCancelActions'] = HelpableActionMap(self, 'OkCancelActions', {'cancel': (self.close, _('Exit mounts setup menu')),
         'ok': (self.ok, _('Select menu entry'))})
        self['ColorActions'] = HelpableActionMap(self, 'ColorActions', {'red': (self.close, _('Exit networkadapter setup menu'))})
        self['actions'] = NumberActionMap(['WizardActions', 'ShortcutActions'], {'ok': self.ok,
         'back': self.close,
         'up': self.up,
         'down': self.down,
...
class NetworkSambaSetup(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'VirtualKeyboardActions'], {'red': self.close,
         'green': self.saveSmb,
         'back': self.close,
...
class NetworkTelnet(Screen):
    ...
    def __init__(self, session):
        ...
        self.skinName = ['NetworkServiceSetup', 'NetworkSamba']
        self.onChangedEntry = []
        self['lab1'] = Label(_('Autostart:'))
        self['labactive'] = Label(_(_('Disabled')))
        self['lab2'] = Label(_('Current Status:'))
...
class NetworkInadyn(Screen):
    ...
    def __init__(self, session):
        ...
        self['key_yellow'] = Label(_('Autostart'))
        self['key_blue'] = Label(_('Show Log'))
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'SetupActions'], {'ok': self.setupinadyn,
         'back': self.close,
...
class NetworkInadynSetup(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'VirtualKeyboardActions'], {'red': self.saveIna,
         'back': self.close,
         'showVirtualKeyboard': self.KeyText})
        self['HelpWindow'] = Pixmap()
        self['HelpWindow'].hide()
    ...
    def updateList(self):
        ...
        self.ina_system = NoSave(ConfigSelection(default='dyndns@dyndns.org', choices=[('dyndns@dyndns.org', 'dyndns@dyndns.org'),
         ('statdns@dyndns.org', 'statdns@dyndns.org'),
         ('custom@dyndns.org', 'custom@dyndns.org'),
         ('default@no-ip.com', 'default@no-ip.com')]))
        if fileExists('/etc/inadyn.conf'):
            f = open('/etc/inadyn.conf', 'r')
            for line in f.readlines():
                line = line.strip()
...
class NetworkuShare(Screen):
    ...
    def __init__(self, session):
        ...
        self['key_yellow'] = Label(_('Autostart'))
        self['key_blue'] = Label(_('Show Log'))
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'SetupActions'], {'ok': self.setupushare,
         'back': self.close,
...
class NetworkuShareSetup(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'VirtualKeyboardActions'], {'red': self.saveuShare,
...
class NetworkMiniDLNA(Screen):
    ...
    def __init__(self, session):
        ...
        self['key_yellow'] = Label(_('Autostart'))
        self['key_blue'] = Label(_('Show Log'))
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'SetupActions'], {'ok': self.setupminidlna,
         'back': self.close,
...
class NetworkMiniDLNASetup(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['key_red'] = Label(_('Save'))
        self['key_green'] = Label(_('Shares'))
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'VirtualKeyboardActions'], {'red': self.saveMinidlna,
         'green': self.selectfolders,
         'back': self.close,
         'showVirtualKeyboard': self.KeyText})
        self['HelpWindow'] = Pixmap()
